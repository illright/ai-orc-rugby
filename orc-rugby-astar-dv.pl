...
perceive(PerceivedCells, X, Y, NewPerceivedCells) :-
  ...
  include(
    within_bounds,
    [X-YUp, X-YDown, XLeft-Y, XRight-Y,
     X-YUpTwo, X-YDownTwo, XLeftTwo-Y, XRightTwo-Y],
    AddedCells
  ),
...
% Case 0.1. Reward successful passes: 0.1
action_heuristic(VisitedCells, PerceivedCells, X, Y, Action, 0.1) :-
  passes(Passes),
...
% Standing at (X; Y) and preparing for Action, rank the cost of the action.
% Case 0.2. Penalize unknown passes: 1000
% We don't want uncertain passes.
...
% Standing at (X; Y) and preparing for Action, rank the cost of the action.
% Case 4. Penalize the visited cells: 500
action_heuristic(VisitedCells, _PerceivedCells, X, Y, Action, 500) :-
  old_action_new(X, Y, Action, XN, YN),
  ord_memberchk(XN-YN, VisitedCells).
...
% The maplist interface for the action_heuristic rule.
heuristic_map(OldPerceivedCells, PerceivedCells, X, Y, Action, Heuristic-Action) :-
  action_heuristic(OldPerceivedCells, PerceivedCells, X, Y, Action, Heuristic).


% Returns the first element of a list without backtracking.
first_member(X, [X | _Rest]).
...
cost(VisitedCells, PerceivedCells, X, Y, Action, Cost-Action) :-
  ...
  old_action_new(X, Y, Action, XN, YN),
  available_actions(Actions),
  maplist(heuristic_map(VisitedCells, PerceivedCells, XN, YN), Actions, WeightedActions),
  keysort(WeightedActions, PrioritizedActions),
  first_member(SecondHeuristic-_BestAction, PrioritizedActions),
  chessboard_distance(X, Y, XN, YN, Distance),
  Cost is 1 / Distance + (Heuristic + SecondHeuristic) / 2.
